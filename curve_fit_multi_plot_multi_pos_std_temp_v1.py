### Curve fitting for multiple plot and multiple leaf position raw files with standard temperature
...
output_folder = 'final/multi_plot_pos_std'
tags_folder = 'tags'
...
output_file = os.path.join(output_folder, 'multi_results_multi_pos_std_temp.xlsx')
...
""" Pick standard temperatures """

### Load temperature reference
temp_df = pd.read_excel('tags/temperature.xlsx')
# print(temp_df)
...
""" END: Pick standard temperatures """
    ...
    for file in os.listdir(input_folder):
        if file.endswith('.xlsx') and '_' in file:
            ...
            try:
                ...
                raw_df = pd.DataFrame({'r_a': a_values, 'r_ci': ci_values, 'r_tleaf': tleaf_values, 'r_patm': patm_values}) # Assemble input data into a dataframe
                # print(raw_df)
                ...
                ### Extract month string and pick standard temperature
                filename_mon = file.split('-')
                if len(filename_mon) >= 2:
                    month_str = filename_mon[1]  # "06"
                    month_name = month_map.get(month_str, None)

                    if month_name:
                        temp_row = temp_df[temp_df['Month'] == month_name]
                        if not temp_row.empty:
                            std_temp = float(temp_row['Temperature'].values[0])
                            # print(f"✅ Standard temperature for {month_name}: {std_temp}")
                        else:
                            print(f"❌ Month {month_name} not found in temperature file.")
                    else:
                        print(f"❌ Month string {month_str} not found in month map.")
                else:
                    print("❌ Filename does not contain expected date format.")

                # #### Extract identifier (e.g., 117 from "2025-06-09-0857_101-114ACi")                
                # filename_parts = file.split('_')
                # if len(filename_parts) > 1:
                #     id_and_rest = filename_parts[1]
                #     id_candidate = id_and_rest.split(' ')[0].split('.')[0]
                #     match = re.search(r'(\d+)-(\d+)', id_candidate)
                #     if match:
                ...
                # Split raw_df into chunks of 10
                chunks = [raw_df.iloc[i:i + 10].reset_index(drop=True) for i in range(0, len(raw_df), 10)]
                # print(chunks)

                """ Calculations based on chunks """



                # Sort each chunk by 'r_ci' ### Jyothi requested not to sort
                sorted_chunks = []
                for ea_df in chunks:
                    sorted_df = ea_df.sort_values(by='r_ci').reset_index(drop=True)
                    sorted_chunks.append(sorted_df)

                for group_df, sheet_id in zip(sorted_chunks, id_range):    ### Sorted, Jyothi requested not to sort previously
                # for group_df, sheet_id in zip(chunks, id_range): ### no sort
                    
                    ### Prepare dataframe for analysing
                    data = {'a': group_df['r_a'], 'ci': group_df['r_ci'], 'tleaf': group_df['r_tleaf'], 'patm': group_df['r_patm']}
                    ...
                    ### Raw plot data Constants
                    # t_leaf = r_t_leaf # °, collected from raw
                    t_leaf = std_temp if pd.notna(std_temp) else r_t_leaf # Used standard temperature, use tleaf mean as a safeguard
                    patm = r_patm # kPa, collected from raw
                    o2 = r_o2 # kPa, collected from raw
                    # print(t_leaf, patm )
                    ...
                    fig1.patch.set_facecolor("#f1cec2")  # Outer area
                    ...
                    axci.plot(plot_dfci['ci'], plot_dfci['a'], 'o-', label='Aobs (Measured)')

                    axci.plot(plot_dfci['ci'], plot_dfci['ac'], 'r--', label='Ac (Rubisco-limited)')
                    axci.plot(plot_dfci['ci'], plot_dfci['aj'], 'g:', label='Aj (RuBP-limited)')
                    axci.plot(plot_dfci['ci'], plot_dfci['at'], 'b-.', label='At (TPU-limited)')
                    axci.plot(plot_dfci['ci'], plot_dfci['calc_a'], 'k-', linewidth=2, label='min(Ac, Aj, At)')
                    axci.set_xlabel('Ci (ppm)')
                    axci.set_ylabel(r'A ($\mu$mol m$^{-2}$ s$^{-1}$)')
                    axci.set_title(f'A/Cc Curve: {file}, , Standard')
                    axci.grid(True)
                    axci.legend()
                    ...
                    print(f"✅ Processed and written: {sheet_name}")

                #     else:
                #         print(f"Skipping file {file}: Could not extract numeric identifier. (e.g., 101-110).")
                #         continue
                # else:
